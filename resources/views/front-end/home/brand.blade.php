	...
	<div class="container">
		<nav aria-label="breadcrumb">
			<ol class="breadcrumb mb-2 small">
				...
				<li class="breadcrumb-item active" aria-current="page">{{ $brand->name ?? __('Brand') }}</li>
			</ol>
		</nav>
		<h1 class="h4 mb-0">{{ $brand->name ?? __('Brand Products') }}</h1>
	</div>
</div>
<section class="section-b-space pt-0 ratio_asos">
	<div class="container">
		...
		<div class="row mb-4">
			<div class="col-12">
				<div class="card border-0 shadow-sm">
					<div class="card-body">
						<form method="GET" action="{{ route('home.brand', $brand->id) }}" class="row g-3 align-items-end">
							<div class="col-md-10">
								<label for="search" class="form-label small text-muted">Search Products in {{ $brand->name }}</label>
							...
							@if(request('search'))
								<div class="col-12">
									<a href="{{ route('home.brand', $brand->id) }}" class="btn btn-outline-secondary">
		...
		@if($products->isEmpty())
			<p class="text-center mt-4">{{ __('No products found for this brand.') }}</p>
		@else
		...
		.then(data => {
			if (data.success && data.wishlist_items) {
				data.wishlist_items.forEach(item => {
					...
					const button = document.querySelector(`.wishlist-btn-${item.product_id}`);
					if (icon && button) {
						icon.classList.remove('ri-heart-line');
						icon.classList.add('ri-heart-fill');
		...
		.then(data => {
			if (data.success && data.compare_items) {
				data.compare_items.forEach(item => {
					...
					const button = document.querySelector(`.compare-btn-${item.product_id}`);
					if (icon && button) {
						icon.classList.remove('ri-refresh-line');
						icon.classList.add('ri-check-line');
