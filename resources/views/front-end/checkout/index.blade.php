...
<section class="section-b-space mt-4">
  <div class="container">
    ...
    <style>
      ...
      .payment-options .pay-tile:hover {
        border-color: #9ca3af;
        box-shadow: 0 2px 10px rgba(0, 0, 0, .06);
      }

      .payment-options .pay-tile .pay-img {
        width: 100%;
        max-width: 100%;
        height: auto;
        max-height: 90px;
      ...
      .payment-options .pay-tile .pay-name {
        font-weight: 600;
        font-size: .95rem;
        color: #374151;
      }

      .payment-options .pay-radio:checked+.pay-tile {
        border-color: #0d6efd;
        ...
        <div class="card">
          <div class="card-body">
            ...
            <div class="d-flex justify-content-between"><span>Subtotal</span><strong
                id="co-subtotal">{{ number_format($cart->subtotal(), 2) }}</strong></div>
            @php($coDiscount = $cart?->discount() ?? 0)
            <div class="d-flex justify-content-between mt-2 text-success" id="co-discount-row"
              style="display: {{ ($coDiscount ?? 0) > 0 ? 'flex' : 'none' }}">
              <span>Discount</span>
            ...
            <div class="mt-3">
              @if($cart && $cart->coupon)
                <div class="d-flex justify-content-between align-items-center" id="co-applied-coupon">
                  ...
                  <button type="button" class="btn btn-sm btn-outline-danger" id="co-remove-coupon-btn"
                    data-url="{{ route('cart.removeCoupon') }}">Remove</button>
                </div>
              @else
            ...
            <div class="d-flex justify-content-between mt-2"><span>Total</span><strong
                id="co-total">{{ number_format($cart->total(), 2) }}</strong></div>
          </div>
        </div>
      </div>
    </div>
